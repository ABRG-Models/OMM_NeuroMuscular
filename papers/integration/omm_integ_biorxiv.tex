% This version of the manuscript is for biorxiv, and does not have the
% Frontiers template.

% While writing, don't stop for errors
\nonstopmode

% The Frontiers-like template
\documentclass{mybiorxiv}
...
% Aliases
\newcommand{\ccg}{Cope-Chambers} % Omit Prescott-Gurney from the name,
                                % too unweildy.
...
% Used colour to indicate changes made following review.
% These commands are now defined to be no-ops
\newcommand{\rvw}[1]{#1}  % Reviewer 1.
\newcommand{\rvwf}[1]{#1} % Reviewer 1, final comment
\newcommand{\rvwt}[1]{#1} % Reviwer 2.
...
navigate within it. It also follows for \e{intrinsic} reasons. For
...
and \rvw{finally the motor neurons}, which innervate the extraocular
muscles
...
individual \rvw{sub-systems} have been proposed for brainstem
...
as the \e{\ccg~model}. This was a rate-coded neural network model
...
as \e{input} the positions of luminances \rvwt{(of fixed shape and
intensity)} on a topographic map. \rvwt{While certain assumptions
were made about the input---that a luminant input excites activity on
a retinotopic layer, with computer code carrying out the
transformation achieved in the brain by a neural connectivity
map \citep{thivierge_topographic_2007}---it is nonetheless \e{not} an
input-assumption model according to our definition because the
activity generated in the neural input layer is modelled as a response
to the luminances, rather than being crafted.}  In the \ccg~model, the
...
reference at each time step. \rvwt{Because a pattern of neural
activity in the output population was assumed to have a behavioural
outcome, it was thus an \e{output-assumption model}}.  The model
...
literature} \citep{gancarz_neural_1998} as the best-of-breed saccadic
...
% Describe what is modelled first. This para is 'The OM system
% performs action selection and the BG will do that for us.'
The brain model, excluding the brainstem, is a re-implementation of
the \ccg~model \rvwt{of reflexive saccadic behaviour \citep{cope_basal_2017}}.
\rvwt{Reflexive saccades are fast eye movements elicited by abrupt
changes in the peripheral visual scene (reflexive saccades can occur
also as a result of auditory and somatosensory stimuli, but these
modalities are ignored in this model). A reflexive saccade has a
starting position defined by the initial orientation of the eye and an
end-point position in which the eye is directed towards a new
target. Regardless of the number of targets within the visual scene,
the brain must choose one location as the end-point, because the eyes
can look only in one direction at a time. The functionality reproduced
by the \ccg~model is `the selection of the best target end-point for a
reflexive saccade'. A competition such as this between incompatible
movements is often referred to as an \e{action selection}
problem \citep{norman_attention_1986,maes_dynamics_1989,redgrave_basal_1999}.
The \ccg~model is therefore a model of action-selection in the
oculomotor system for reflexive saccades.  One hypothesis for the
r\^ole played by the basal ganglia (BG) is that the system
performs \e{action selection}
\citep{mink_basal_1996,redgrave_basal_1999,hikosaka_role_2000}.
The \ccg~model places the BG at the centre of the oculomotor system;
this follows the known anatomy of the
region \citep{hikosaka_role_2000} and provides a mechanism for action
selection of the best saccade. The BG receives input indirectly from
the superior colliculus, which has a retinotopic
arrangement \citep{ottes_visuomotor_1986}.}

% This para describes some anatomical stuff, and talks about what was
% omitted from the ccg model.
\rvwt{The BG receives excitatory inputs directly from retinotopic regions of
the cortex including the frontal eye fields (FEF), supplementary eye
fields (SEF), lateral intraparietal cortex (LIP) and dorsolateral
prefrontal cortex. The dorsolateral prefrontal cortex, which
participates in voluntary saccades
\citep{funahashi_prefrontal_1993,munoz_look_2004}, is not
modelled because the model concerns reflexive rather than voluntary
eye movements. Several other regions of the brain that are associated
with eye movements are also omitted from the model. The early visual
processing stream in cortex, from V1, through to the LIP is subsumed
into a `sustained retinal' signal which arrives at FEF. The
justification here is that the model reacts to simple luminant targets
and does not need to carry out the feature extraction performed by
these visual areas. The supplementary eye fields are involved in the
programming of saccade sequences \citep{tehovnik_eye_2000} and memory
guided
saccades \citep{chen_supplementary_1995,schlag_neurons_2002}. Lesions
of SEF do not affect visually guided
saccades \citep{gaymard_cortical_1998} and so the SEF is also omitted
from the model.}
...
\end{center}
\caption{ \bol{a) The macroscopic architecture of the \ccg~model.} The main
...
single neural population. The BG box is expanded in: \bol{b) The basal
ganglia model component.}
%
...
to its targets. \bol{c) 2D Gaussian weights.} The `GaussianKernel'
...
otherwise no connection is made. \bol{d) Gaussian connectivity.} This
...
\e{values}, which reduce `towards the edge of the circle' and follow the
relationship shown in (c).}
\label{fig:ccg_model}
\end{figure}

% Now describe each bit a little more, with help of the CCG
% macroscopic architecture figure.
\rvwt{Fig.~\ref{fig:ccg_model}(a) shows the macroscopic architecture of
the \ccg~model. The figure shows the relationships between the retinal
input populations, the FEF, the populations comprising the BG
sub-system (the red border indicates that the box represents a number
of populations as a sub-system), the thalamus and the superior
colliculus. Excitatory connections are indicated with arrowheads;
inhibitory connections with circles in place of the arrowheads. The
blue and green connection lines indicate two thalamo-basal ganglia
loops, one cortical loop through FEF (green), the other a sub-cortical
loop through SC. It is important to note that although they are given
different colours in the diagram, these loops are in no way
independent, with loop activity combining both in thalamus and in the
basal ganglia and a direct excitatory, feed-forward connection from
FEF to SC.}

% More on BG model and mechanism; GPR recap
\rvwt{The basal ganglia sub-system is the most complex component of
the \ccg~model. The BG model is based on previous
work \citep{gurney_computational_2001,gurney_computational_2001-1} and
is referred to as the GPR model. The GPR model incorporates the
following main components of the primate
BG \citep{mink_basal_1996,wickens_basal_1997}: (i) The striatum (the
main input station to the BG) which is divided into two iterdigitated
populations of projection neurons expressing primarily D1 or D2-type
dopaminergic receptors (named Str\_D1 and Str\_D2); (ii) The
subthalamic nucleus (STN); (iii) the external segment of the globus
pallidus (GPe); (iv) the output nucleus relevant for saccadic
control---the substantia nigra pars reticulata
(SNr) \citep{hikosaka_role_2000}.}

% More GPR recap
\rvwt{The connectivity of the GPR model [Fig.~\ref{fig:ccg_model}(b)]
is constrained by the known anatomy and physiology of the
BG~\citep{bolam_synaptic_2000}. Physiologically, the only source of
glutamate within the BG is the STN, whose projections are therefore
excitatory; all other nuclei have GABAergic projection neurons and are
therefore inhibitory. The cortex sends glutamatergic projections to
both the Str\_D1 striatal population, which projects preferentially to
the SNr, and to Str\_D2, which projects primarily to
GPe \citep{gerfen_d1_1990}. The cortex also projects to the STN, which
sends diffuse projections to the SNr and
GPe \citep{parent_anatomical_1993}. The GPe projects to the SNr and
also projects back to the STN, completing a GPe--STN loop.}

% GPR model and action channels
\rvwt{The GPR model is arranged into `action channels';
Fig.~\ref{fig:ccg_model}(b) shows an example network containing three
channels. It is between these channels that competition occurs, with
the winning channel succeeding in reducing activity in the output
nucleus, SNr, and thereby disinhibiting its target. The complete
connectivity pattern for this small network is shown in
Fig.~\ref{fig:ccg_model}(b); the left channel in cortex innervates the
left channels of Str\_D1, STN and Str\_D2. Connections are one-to-one,
so it follows that the middle channel of cortex innervates the middle
channels of STN and the striatal populations and the right channel of
cortex innervates right channels in striatum and STN. Striatal
population channels also inhibit SNr and GPe on a one-to-one basis and
GPe feeds inhibition to SNr and STN in a one-to-one manner. The
outputs from STN however are not one-to-one. The output from all
channels of STN is summed together and then the sum is fed into each
channel of SNr and GPe. This models the diffuse excitation from STN
which has been observed in the BG \citep{parent_anatomical_1993}.}

% BG selection mechanism in GPR model
\rvwt{Within the BG, there are several mechanisms supporting
competitive processing for selecting channels whose inhibitory output
should be reduced. The selection mechanism of the GPR model is the
`off-centre, on-surround' scheme proposed
by \cite{mink_basal_1993}. The `on-surround' is provided by
diffuse, excitatory projections from the STN to the SNr. Focussed
inhibition from the Str\_D1 neurons in striatum contributes the
`off-centre' part of the mechanism. This arrangement leads to
selection behaviour via a release of target inhibition, since
channels that have strong salience (input) have weak output at the
level of SNr, and channels with weak salience have enhanced output.}

% BG mechanism - GPe's role
\rvwt{The GPe is not included in the centre-surround circuit described
above, but still plays a key r\^ole in selection. Operating alone, the
Str\_D1/STN/SNr circuit can suffer from the following problem: if the
input for all channels is relatively high, then the diffuse projection
from STN, which effectively supplies a sum of \e{all} of the STN
inputs to each channel in SNr, will provide so much excitation that
Str\_D1 may become unable to inhibit one of the channels in SNr and
selection may become
impossible. \cite{gurney_computational_2001,gurney_computational_2001-1}
showed that the inhibitory feedback from GPe to STN acts as an
`automatic gain control' to help prevent this from occurring.}

% Dopamine, tonic output.
\rvwt{At the neuronal level, the STN, GPe and SNr have tonic output
activity \citep{chevalier_disinhibition_1990,delong_primate_1985,kita_intracellular_1991}. This
is modelled using piecewise linear output functions with zero offsets,
$c$ (see Eq.~\ref{eq:lintransfunc}) but with noise added to the
input. In striatum, Str\_D1 and Str\_D2 have positive offset $c$,
mimicking the so-called `down-state' of medium spiny neurons which
have a resting potential far below spiking threshold and require
co-ordinated input to generate action
potentials \citep{wilson_origins_1996}.
% Note, in Alex's paper, there is an error with the offsets for D1,
% D2. These are set to -0.05 instead of 0.05. A quick trial with the
% model suggests this doesn't make a very large difference to the
% overall operation (you just see a background of noise around the
% signals in Str), and I have the sign correct in any case.
In addition, the Str\_D1 and Str\_D2 neurons are influenced by
dopamine in different ways; facilitating cortico-striatal transmission
at medium spiny neurons with D1
receptors \citep{hernandez-lopez_D1_1997,gonon_prolonged_1997} and
reducing transmission at those with D2
receptors \citep{delgado_inhibitory_1999}. These effects are modelled
using a dopamine parameter, $d$, which modulates the input activations
$a_{in}^{D1}$ and $a_{in}^{D2}$ as:
\begin{equation}
   a_{in}^{D1} = (0.2 + d)A
\end{equation}
\begin{equation}
   a_{in}^{D2} = (1 - d)A
\end{equation}
where $A$ is the input activation (see also
Eqns.~\ref{eq:a_in_d1} \& \ref{eq:a_in_d2}). For the `normal, healthy'
value for $d$ of 0.7, Str\_D1 activation is relatively enhanced
(0.9$A$); Str\_D2 activation is one third of this value (0.3$A$). The
major effect of this difference in the relative strength of the
activity in Str\_D1 versus Str\_D2 is simply that a change in the
level of activity in Str\_D1 affects the off-centre, on-surround
mechanism. The effect of varying the input into Str\_D2 is much more
subtle, with only a small change in the amount of inhibition fed from
GPe into STN (via a focussed, one-to-one connection) being affected by
the change, along with a small change in the inhibition fed into SNr
from GPe (also via a one-to-one connection). It is not possible to
ascribe the dynamic effect of the dopamine parameter to any single
population, because the activity is recurrently
connected through multiple loops. Thus, a line of reasoning such as
``reduced activity for a luminance in Str\_D2 will lead to less
inhibition in that region in GPe, which means that there will be
higher activity there, and hence more inhibition for that region
passed to STN leading us to expect less activity in STN'' is verified
by running a suitable simulation with the model, but the effect is
small. }
%
\rvwt{Note that
the effect of dopamine in the model is only to modulate the strength
of cortico-striatal synapses; no learning is modelled and so the
significance of dopamine as a prediction error signal is outside the
scope of the current work.}

% Change this - we already talked about channels in the context of the
% three channel model shown. Now talk about how we have a map of
% channels - a continuous space in the \ccg model. This can then lead
% on to Fig 3.
\rvwt{The GPR model in Fig.~\ref{fig:ccg_model}(b) has only three
channels, with the focussed inhibition from striatum to SNr and GPe
defined by a simple one-to-one scheme. The action channels represent
discrete, incompatible motor action choices. In the oculomotor model,
an action channel represents the end-point of saccade, and the
competition carried out in the basal ganglia is between potential
saccade end-points. However, eye movements have a \e{continuous}
end-point space; the eye can rotate to any orientation within its
biomechanically permissible range. Some end-points within this range
are mutually exclusive---it's not possible to look to the left and to
the right simultaneously---but \e{nearby} end-points are not
necessarily incompatible. A small enough error in the end-point of a
saccade will not prevent the eye from foveating on a target as the
foveal region of high visual acuity is not infinitesimally small. To
cope with this requirement, the populations within the oculomotor
basal ganglia are conceived of as two-dimensional topographic grids of
leaky integrator neural elements. Activity in each element corresponds
to a spatial location in the visual field. Neighbouring elements
correspond to locations which are close to each other in the visual
field. Focussed one-to-one projections in the GPR model are replaced
by projective fields with many weighted connections. Specifically,
each unit in Str\_D1 projects to a counterpart SNr$_j$ in SNr with
some weight $w_{max}$, but also connects to neighbouring nodes in SNr
with a weight given by $w_{max}.G(d)$, where $G(d)$ is a circularly
symmetric, 2D-Gaussian which is a function of distance $d$ from
SNr$_j$ [Figs.~\ref{fig:ccg_model}(c)\&(d)]. A similar scheme applies for
the connectivity from Str\_D2 to GPe and for a number of the other
connections in the \ccg~model; in the SpineML implementation of the
model, this connectivity scheme is named `GaussianKernel'.
% NB - having this Fig. ref here is important for figure ordering!
Fig.~\ref{fig:brainmodel} shows a schematic of the SpineML implementation
of the model, based on a diagram produced by SpineCreator. Populations
for Str\_D1, Str\_D2, STN, SNr and GPe are
shown within the `Basal Ganglia' box. Input comes into the model via
the `World' population and the output population is SC\_deep. Compare
this diagram with Figs.~\ref{fig:ccg_model}(a) \&
(b). Fig.~\ref{fig:brainmodel} expands the `SC', `BG' and `slow retinal'
boxes from Fig.~\ref{fig:ccg_model}(a).}
...
populations. \bol{a)} Fixation cross input in Cartesian eye
frame. \bol{b)} Fixation cross input in retinotopic
co-ordinates. \bol{c)} Fixation cross with noise added. \bol{d)}
After blurring, input is passed to BG populations.}
\label{fig:brainmodel}
\end{figure}

% Now some FEF stuff taken from Alex's pre-pub
\rvwt{The frontal eye fields (FEF) are a key cortical area for the
generation of saccadic eye movements
\citep{hikosaka_role_2000,tehovnik_eye_2000,robinson_eye_1969,bruce_primate_1985}.
Saccadic targets are retinotopically mapped on its
surface \citep{robinson_eye_1969,bruce_primate_1985,sabes_parietal_2002},
and increased neural activity at a location on the map precedes a
saccade to that location. Importantly, the FEF is also associated with
visual decision making
\citep{thompson_visual_2005,schall_saccade_1995,monosov_measurements_2008,cohen_neural_2009}.
Thus, in a saccade choice, increased FEF activity is predictive of the
eye movement whether correct or
incorrect \citep{thompson_frontal_2005}, rather than of the correct
response.}

\rvwt{FEF neurons can be divided into three functional groups,
related to whether their activity corresponds with visual stimuli,
motor action, or both \citep{segraves_functional_1987}. The \ccg~model
simplifies this categorisation using a single layer of 50 by 50 units
representing the mean of all three groups. This layer therefore
responds to both visual stimuli and the buildup of activity associated
with motor (saccadic) action. The retina provides a persistent
luminance signal into the FEF through the dorsal visual
pathway \citep{ungerleider_two_1982} which is abbreviated in this
model to a direct connection with delay and noise.}

\rvwt{The FEF provides input into the
BG \citep{saint-cyr_organization_1990} (to Str\_D1, Str\_D2 and STN)
which, in turn, projects back to thalamus in a retinotopically
organised way \citep{middleton_basal_2000,lynch_input_1994}. In
addition, the thalamic targets of this path are regions with strong
reciprocal connections to the FEF \citep{mcfarland_thalamic_2002}. In
this way, the FEF forms channel-based loops through basal ganglia of
the kind described above. Such circuits formed the basis of the model
of \cite{humphries_role_2002}.
%In that model, the
%thalamic component included the thalamic reticular nucleus which
%implemented an additional selection mechanism (via a feed-forward
%competitive network within the cortico-thalamic loop). However, in the
%interests of model simplicity, this was not implemented in the \ccg
%oculomotor architecture.
The thalamo-cortical loop may be thought of as an integrator of
information, whose gain is modulated by inhibition from basal
ganglia \citep{chambers_mechanisms_2012,cope_biologically_2011}.}

\rvwt{The superior colliculus (SC) is a sub-cortical nucleus which
also plays a critical r\^ole in the generation of
saccades \citep{hikosaka_visual_1983}. Both FEF and SC have direct
connections to the saccadic burst generator (SBG, see
Sect.~\ref{sec:methods:brainstem}). If either is lesioned, the other
can direct gaze, following a period of
adjustment \citep{latto_effects_1977}, albeit with some persistent
deficits. The SC is also a direct target of output from the
SNr \citep{jayaraman_nigrotectal_1977,jiang_opposing_2003} and can be
influenced by the action selection mechanisms of the BG. In
particular, it forms a loop with BG, but unlike its cortical
counterpart in FEF, the input to basal ganglia comes via the thalamus
[Fig~\ref{fig:ccg_model}(a), blue arrows].}

\rvwt{While the SC has seven alternating cell-~and fibre-rich
layers \citep{wurtz_visual-motor_1980}, in most cases these are
categorised as the `superficial' and `deep' layers, which have
significantly different response properties. Cells in the superficial
layers, which receive input from the retina, are mainly visually
responsive, with a preferred response to phasic events (luminance
onsets and offsets) and movement on the visual
field \citep{goldberg_activity_1972}. In contrast, cells in the deep
layers receive multi-modal input, including inhibitory input from the
output structures of the BG \citep{jayaraman_nigrotectal_1977}, and
are directly involved in the generation of saccadic eye
movements. Saccade related activity in the deep layers appears to
generate saccades through `population coding', with a weighted sum of
activity across the retinotopy of SC determining the saccade target
\citep{lee_population_1988,van_opstal_role_1990,mays_dissociation_1980}.
The deep layers of SC receive input from the FEF in a topographic
manner \citep{stanton_frontal_1988,sommer_composition_2000}.}

\rvwt{The SC in the \ccg~model is based on the the model described
in \cite{arai_model_2005}, with the difference that the SNr input to
the SC is generated by the BG model, rather than being
hand-crafted. The SC model has a superficial and a deep layer, each of
which is a 2-D array of 50 by 50 leaky integrator units arranged in
the same retinotopic manner as the
FEF \citep{wurtz_visual-motor_1980}.}
...
co-ordinates to retinotopic co-ordinates. \bol{a)} The mapping of
...
simulations, to make these images clearer. \bol{b)} The locations of the
...
between the two crosses. \bol{c)} The FEF\_add\_noise populations adds a
level of noise to the signal representing processing of the signal in
visual cortex. \bol{d)} A Gaussian projection from FEF\_add\_noise to FEF
...
% Foveal mask
\rvwt{The \ccg~model incorporates a special connectivity pattern for
visual input to the BG via cortical (FEF) and sub-cortical (thalamus)
pathways. Due to the retinotopic mapping (Sect.~\ref{sec:mapping}),
foveal luminances deliver a strong signal to the BG; roughly one third
of the map is activated for the foveal targets used in this work
(Fig.~\ref{fig:mapping}, red cross). This makes it virtually impossible
for a peripheral target (Fig.~\ref{fig:mapping}, yellow cross) to win
selection in the BG. Even if the peripheral target competed
successfully to generate a saccade, this process would cause a
significant delay, leading to latencies much larger than those
observed experimentally. To overcome this problem, the \ccg~model
incorporates a mechanism in which the synaptic strength of connections
between FEF, thalamus and striatum are reduced close to the fovea
according to a shifted hyperbolic tangent. This connection is named
`DecayingAtFovea' in the SpineML implementation and follows a modified
sigmoidal curve rather than tanh. In either case, the relation is
`S-shaped' and normalised to the range [0 1]. Far from the fovea
(where the S-shaped curve has the value $\approx$1), the connectivity
pattern looks almost identical to a one-to-one connection.}

% Retinal model
\rvwt{Input to the \ccg~model is provided through a simple retina
model which directly samples from a larger `world array' of pixel
values. In the current model, the input for the retina is named
`World' and is the retinotopic projection [Fig.~\ref{fig:brainmodel}(b)]
of the eye's field of view of the world [Fig.~\ref{fig:brainmodel}(a)]
and the luminant targets therein. The raw input in `World' is fed into
a population which adds noise, and then via a delayed connection to
FEF (the sustained retinal input path), to simulate processing through
the dorsal visual stream. It is also fed, without substantial delay,
into two leaky integrator layers (Retina\_1 and Retina\_2) with
different time constants, with the more slowly reacting layer
(Retina\_2) inhibiting its faster counterpart. The faster layer
responds quickly to the appearance of a prolonged stimulus before it
is inhibited by the slow layer, forming a phasic response to stimulus
onset. The mechanism ensures that phasic rather than tonic responses
arrive at the superficial SC from the retina.}

% CCG model output
\rvwt{The output of the \ccg~model is determined by the activity in the
SC\_deep population. The activity in SC\_deep is first transformed
from retinotopic co-ordinates into Cartesian co-ordinates. The
centroid of the activity is then computed. The position of this
centroid in the Cartesian frame determines the saccadic end-point. The
current model differs in that it does not compute a centroid, instead
feeding the SC\_deep activity into the saccadic burst generator.}

% Parameterisation here?
\rvwt{The \ccg~model was parameterised by tuning the model to perform
a prosaccade task in which a central luminance point was fixated by the
model. After a fixed duration, the fixation point was extinguished and
a target point of fixed luminance was presented. The model was tuned
so that the latency between the presentation of the target and the
initiation of an eye movement matched experimental
data \citep{reulen_latency_1984}, while also matching the
electrophysiological evidence of activity in a variety of brain
regions. The tuning of the BG model attempted to preserve as closely
as possible the weights used in the original paper
\citep{gurney_computational_2001}. Further details on
the parameterisation of the \ccg~model are given in \cite{cope_basal_2017}.}
% Differences between ccg and existing brain model or can I do that
% above? Better in the preceding text, I think and wherever it crops
% up.
...
a first order differential equation specified in a \e{SpineML
component}. SpineML, which will be outlined in
...
The \e{LINlinear} component, which is used in FEF, Thalamus, SC, SNr
...
function. \rvwt{If $c<0$, then for zero activation ($a=0$), the output
will be positive. This simulates the effect of a neural population
having tonic firing. If $c>0$ then the output will be zero until the
activation exceeds $c$, simulating neurons which only fire when driven
by excitatory input.}

The \e{LINret} component used for the retinal populations is similar to
...
\begin{equation}
   a_{in} = A
\end{equation}
The \e{LINexp} component is a leaky integrator with an exponential
...
The \e{D1MSN} and \e{D2MSN} components are both leaky integrators, similar to
...
$i^{th}$ activation or shunting connections \rvwt{received by the component};
...
% Was looking for somewhere for this, possibly:
%The precision of the model to distinguish between
%targets, especially towards the periphery of the field of view was
%limited by the 50$\times$50 size of the neural layers. In order to optimise
%this precision, both the \ccg~model and our implementation of
%it had their effective eccentricity range limited. Our implementation
%was limited more than the Cope-Chambers model, because hills of
%activity in SC_deep had to be kept fully within each layer. When a
%hill `falls off the edge' of the layer, the amount of activation being
%passed to the SBG is reduced and the saccades become inaccurate. I
%didn't want to investigate effects at the edge of the SC, so I had an
%effective limit of the model of about 22 degrees of eccentricity for a
%saccade.
...
Fig.~\ref{fig:sbg}. \rvw{In the brainstem model, we use the word `channel'
to mean a set of populations of neurons which are involved in
actuating a single extraocular muscle. SBG channels are arranged in
pairs, actuating opposing muscles. There is one pair of channels which
actuates the superior and inferior rectus muscles, causing vertical
rotations of the eye in a roughly parasaggital plane (the eye moves up
or down). Another pair actuates the lateral and medial rectus muscles,
...
The output signals of the brainstem's \rvwt{motoneuron (MN)
populations} are used to drive the biomechanical model. \rvwt{The MN
output signal in each brainstem channel is normalised in the range [0
1] and represents the mean firing rate of the neurons that innervate
the extraocular muscle for that channel.} The biomechanics are used
not only to get tangible feedback on the simulated saccades including
motion trajectories, but to add one more modelling dimension related
to the inertial properties of the eye plant including muscle
properties.
...
\end{center}
\caption{ The biomechanical eye. \bol{a)} an OpenSim rendering of a pair of
...
their respective muscle pulleys. \bol{b)} OpenSim rendering of biomechanical
eye within a head model \bol{c)} Top-down schematic cross-sectional view of
...
% KM
\rvwt{An illustration of the biomechanical eye model is given in
Figure \ref{fig:bioeye}(a), while Figure \ref{fig:bioeye}(b) depicts
the head model used in the proposed
framework. Figure \ref{fig:bioeye}(c) shows a cross-sectional view of
the eye and the spherical screen on which targets were projected.}

% KM
\rvwt{Two types of muscle models of different complexity are supported. The
first models muscles using linear path actuators. This simplistic
model of ideal muscles can be easily integrated with high level brain
models. As described above the muscles are wrapped around the eye.
The more complex model supported is based on the Thelen model
\citep{thelen_adjustment_2003} that is also supported by OpenSim and implements
Hill-type muscles. It includes realistic muscle wrapping geometric
entities of the muscle fibers, while it accommodates for both
activation and contraction dynamics.  The dynamics of muscular forces
can be split into: 1) The elasticity of the muscles. 2) A delay
...
potentials and by the necessary calcium release at the muscle fibres.}
...
% KM
\rvwt{Finally, numerical integration of the biomechanical eye model is based
on the Kutta-Merson integration method.}
...
\end{center}
\caption{ The model framework. \bol{a)} The model is specified using a combination of
declarative XML files and hand-coded C++. These original model
specifications are shown within the blue box. \bol{b)} The green box shows
...
BRAHMS executable to locate them at runtime. \bol{c)} The red box shows the
...
\rvwt{Thus, the information flow in the model is as follows:
Luminances in the world have their locations computed in the eye's
frame of reference, based on the rotational state of the eye. The
locations of the luminances on the retina are transformed into a
retinotopic co-ordinate system which determines the activity in the
`World' population (named to mean the `world as the brain sees it',
rather than the world frame of reference) which is the input for the
brain model. The target luminance for a saccade is selected, as
described earlier, via cortical and sub-cortical loops through the
basal ganglia model and activity for the winning end-point builds up
in the deep layer of superior colliculus. This activity excites the 6
channels of the saccadic burst generator in the correct proportions
for the saccade. The motoneurons, which are the output of the SBG,
send a rate-code signal (normalised between 0 and 1) into the
biomechanical eye model. The rotational state of the eye model is fed
back to participate in the computation of the retinotopic luminance
activity in `World', completing the loop.}
...
\rvwt{Although there is no definitive experimental proof for such a mapping,
there exists evidence for spatially variable synapse
density \citep{moschovakis_anatomical_1998,herrero_influence_1998} and
connection density \citep{grantyn_density_2002} and we therefore adopt
the idea.}
%
...
profiles~\citep{arai_two-dimensional_1994}---\rvwt{that is, they
assumed that the activity in superior colliculus for any saccade was a
size-invariant 2D Gaussian hill of activity}.
%
...
map, \rvwt{obtained by inverting the mapping} of \cite{ottes_visuomotor_1986}
and the assumption of invariant 2D Gaussian activity profiles in SC,
\rvwt{which is equivalent to:}
%
\begin{equation} \label{eq:tabareau_eq3}
w(r,\phi) = i\;e\,^{jr}\;\sin\left(l\phi + k\right)
\end{equation}
\rvwt{where $r$ and $\phi$ are co-ordinates on the collicular map and
$i$, $j$, $k$ and $l$ are parameters of the
function (compare with Eq.~3 of \cite{tabareau_geometry_2007}).}
%
...
\end{equation}
which is invariant for all $\mathbf{z}$. \rvwt{This requirement is
fulfilled by spatially invariant 2D Gaussian profiles, whose
time-course (how quickly they grow and then diminish) is always the
same.}

However, the very mapping on which the \cite{tabareau_geometry_2007}
result is based leads to a very \e{variant} activity profile \rvwt{in
the SC\_deep layer} of the \ccg~model. A luminance of a given size
which excites activity near to the fovea causes activity in a large
number of neurons in each retinotopic layer, whereas activity far from
the fovea excites a much smaller region. This effect is clearly
demonstrated in Fig.~\ref{fig:mapping} for equal sized targets both on
and distal from the fovea.
...
\end{center}
\caption{ \bol{a)} \& \bol{b)} Tabareau-style weight maps for `left' and `down' components
of a saccade. \bol{c)} The two weight maps in (a) and (b) shown on the same
...
red circles encode saccades with both a left and a down component. \bol{d)}
...
% Try to make this clearer for Reviewer 2, Methods Comments, point 5.
\rvwt{To understand the need for this invariance,
consider the effect of a 2D Gaussian hill in SC\_deep which elicits a
successful horizontal saccade of 10\dg~using the weight maps shown in
Figs.~\ref{fig:scdeep}(a)--(c). Activity from the 2D Gaussian
(schematically represented as the large purple dashed circle in
Fig.~\ref{fig:scdeep}(c)), passing through the weight maps will excite
the superior (not shown) and inferior rectus
[Fig.~\ref{fig:scdeep}(b)] channels by an equal, balanced amount, so
these cancel out, allowing the eye movement to be horizontal. The
amount of activation passed to the lateral rectus muscle results from
a convolution of the Gaussian and the exponential component of the
weight map relationship in Eq.~\ref{eq:tabareau_eq3}. If the Gaussian
hill now appears further along the collicular surface, coding for a
20\dg~saccade, \e{and also becomes smaller} (small purple dashed
circle), we can still argue that the vertical component signals to
superior/inferior rectus muscles will cancel out, and we could imagine
that the exponential component of Eq.~\ref{eq:tabareau_eq3} is
correctly parameterised to compensate for the smaller hill. Now
consider a 2-D Gaussian hill which codes for a 10\dg~saccade which is
`down, and to the left' in equal proportions (large red circle). That
means that the hill will sit on the boundary between the weight maps
for the `down', and the `left' muscles. Now, if the hill moves to the
$r=\mathrm{20}$\dg~location on the colliculus (small red
circle), \e{and also reduces in size}, it will excite only the
periphery of the sine; the exponential increase of the map along $r$
is not guaranteed to compensate for the reduction in the convolution
of the Gaussian hill and the sinusoidally varying component of the
weight map along the $\phi$ axis in Fig.~\ref{fig:scdeep}(a).}
% Add figure for this.

This led us to hypothesise that the retinotopic mapping \rvwt{to the
SBG be preceded} by an associated widening projection field such that
the hill of activity in \rvwt{a `final' deep layer of superior
colliculus is invariant with position on the map}. There are a number of
...
present evidence for another projecting field within SC whose
...
% Could elaborate here on Listings Law, and why I didn't train to
% Listings Law. Do this in the chapter, rather than here in the paper.
...
them ready for the next saccade. \rvw{There is evidence for inhibitory
projections to SC from the propositus
hypoglossi nucleus \citep{corvisier_possible_1991}, which lies within the
brainstem, upstream from motoneurons, and has been shown to encode eye
velocity \citep{dale_nucleus_2013}.}
...
Fig.~\ref{fig:saccvstarg}(a) \& \rvw{(b)}. We applied the same parameters to
...
0.2 and target luminance 0.3. \bol{a)} Horizontal rotations about the
y axis. \bol{b)} Vertical rotations of the eye about the x axis.}
\label{fig:saccvstarg}
\end{figure}

The resulting weight maps (where the oblique maps are
1/10$^{\mathrm{th}}$ of the vertical maps, as described earlier) are
shown in Fig.~\ref{fig:weightmaps}.
%
\rvw{First, recall that the $r$ axis of the neural surface corresponds
to the amplitude of a saccade and the $\phi$ axis indicates the polar
direction of the saccade, as described in Sect.~\ref{sec:mapping} and
Fig.~\ref{fig:mapping}. Fig.~\ref{fig:weightmaps}(a) shows the weight
map for the muscle which rotates the eye to the left. As we modelled a
left eye, this actuates the lateral rectus muscle. The exponential
rise of Eq.~\ref{eq:weightmaps} [for experimental evidence,
see Figs.~7 \& 8 of \cite{herrero_influence_1998}] is seen in the
$r$ direction; as $r$ increases, so the connection strength to the SBG
channel rises exponentially. The connection strength is greatest along
the centre line, for a value of $\phi$ which corresponds to a purely
leftward movement. Note that $\phi$ is presented in neural
co-ordinates, and not in degrees or radians; $\mathrm{1}\leq\phi\leq\mathrm{50}$
corresponds to a range of 0\dg~to 360\dg; $\phi=\mathrm{38.5}$ corresponds to
movements left. The connection strength drops away sinusoidally as
$\phi$ moves away from the centre line at $\phi=\mathrm{38.5}$. In regions of
the map for which there is no leftward movement, that is, in the half
of the map which corresponds to any movement with a rightward
component, the `left' weight map is 0. Fig.~\ref{fig:weightmaps}(d)
shows the weight map for rightward movements, actuating the medial
rectus muscle of the eye. The line of maximum connection strength is
along $\phi=\mathrm{13.5}$. The map is a mirror of
Fig.~\ref{fig:weightmaps}(a), reflected about the line $\phi=\mathrm{26}$. The
`left' and `right' weight maps are orthogonal; the non-zero region of
the `left' map is zero in the `right' map and vice
versa. Fig~\ref{fig:weightmaps}(b) \& (d) show the weight maps for
downward and upward eye movements; the `down' map activates the SBG
channel for the inferior rectus muscle, the `up' map activates the
superior rectus. Note that `down' is not orthogonal either to `left'
or `right' because a saccade down and left is achieved by
simultaneously activating both the lateral and inferior rectus
muscles. However, the `up' map \e{is} orthogonal to the `down' map and
spans the edges of the surface where $\phi$ rolls over from 1 to
50. The line of maximum connection strength for the `up' map is along
$\phi=\mathrm{1}$; for `down' $\phi=\mathrm{26}$. Based on the training described in
Sect.~\ref{sec:methods:integrating}, the maps driving the superior
oblique (`Z+') and inferior oblique (`Z-') muscles were set to
1/10$^{\mathrm{th}}$ of the `down' and `up' maps.}
...
multiplied by cosine($\phi$) about its `active' axis. \bol{a)} Weight
map for leftward movements which innervates the lateral rectus in this
single left-eye model. \bol{b)} Map for downward movements; innervates
the inferior rectus. \bol{c)} Superior oblique muscle weight
map. \bol{d)} Weight map for rightward movements/medial rectus
muscle. \bol{e)} Weight map for the superior rectus muscle which
generates upward movements. \bol{f)} Inferior oblique map.}
...
(TModel3). \bol{a)} The ratio of the magnitudes of the total error vector
and the target vector, expressed as a percentage. \bol{b)} The ratio of the
magnitude of the $x$ component of the error vector to the magnitude of
the target vector, expressed as a percentage. \bol{c)} As (b) but for the
$y$ component. \bol{d)} As (b), for $z$ component. All colour maps are shown
...
colliculus. \rvw{As described in
Sect.~\ref{sec:methods:integrating}, there is
...
projection field was added to the model of the superior colliculus. \bol{a)}
The ratio of the magnitudes of the total error vector and the target
vector, expressed as a percentage. \bol{b)} The ratio of the magnitude of
the $x$ component of the error vector to the magnitude of the target
vector, expressed as a percentage. \bol{c)} As (b) but for the $y$
component. \bol{d)} As (b), for $z$ component. All colour maps are shown
with the same scale. The target rotations, $\theta_{x}^t$ and
$\theta_{y}^t$ are denoted `Target X' and `Target Y' in the figure.
...
Figs.~\ref{fig:errorsurface}(a)--(d) show the \rvw{percentage errors for
TModel4}.  First of
...
\end{center}
\caption{ Representative single saccades. \bol{a)} Trajectories from 9 saccades to a
...
closest to the end of the trajectory. \bol{b)} The three rotational
components of the `dark blue' saccade, to target location (-7,-7). \bol{c)} The
...
Fig.~\ref{fig:mapping}). Closer to the fovea, the \rvwt{effect of} the
foveal mask \rvwt{on} the activity in FEF again leads to reduced input
...
\end{center}
\caption{ Exploring saccade latencies. \bol{a)} Latency to first movement as a
function of target eccentricity for horizontal targets. \bol{b)} Latency
vs. eccentricity for vertical targets. \bol{c)} Latency vs. gap at three
...
luminance used here. \bol{d)} The effect
of the dopamine parameter on saccade latencies in gap, step and
overlap conditions, for two different target luminances.  \bol{e)} Saccade
...
(\cite{bahill_trajectories_1979}, p.~6). \rvw{Had the return saccades not
...
original fixation. \bol{a)} Out and return saccade to a target at
(0,-10\dg). The outward trajectory is shown in blue, the return in pink.
\bol{b)} Rotational components of the saccade shown in (a). \bol{c)} Outward and
...
of centre). \rvwt{The stimuli could not be presented at 15\dg~and
30\dg~to match the experiment, because 30\dg~saccades were outside the
range of the model.}
...
12\dg, followed by a second target at 12\dg~or 8\dg. \bol{a)} Horizontal
...
mean. \bol{b)} The $\theta_{x}$/$\theta_{y}$ trajectories corresponding to
...
a musculo-skeletal system, and then \rvw{closing the
`agent-environment-agent' loop} by allowing the
...
(Fig.~\ref{fig:errorsurfaceTM3}). Thus, \e{the integration of the models}
...
lines] the second saccade latency was \e{longer} even than the initial
...
% Inhibition experimental evidence
\rvw{There is some evidence for an inhibitory projection to SC from the
brainstem. \cite{corvisier_possible_1991} offer evidence for a
projection from the propositus hypoglossi nucleus. This lies upstream
from motoneurons and (in primates) encodes eye
velocity \citep{dale_nucleus_2013}, rather than head movement
velocity. Although the propositus hypoglossi does not lie in exactly
the same functional location as our IBN population (instead it sits
between TN and MN), it offers a possible inhibitory feedback signal
proportional to eye velocity and may help to reduce activity in SC
post-saccade.}
%
\rvwt{Alternatively, it is possible that activity in
FEF and thalamus are reset via a `timed signal'. Feasibly, after
activity in FEF exceeds a threshold, an internal, inhibitory feedback
signal could be activated. This inhibition should have a timescale of
sufficient duration to reduce activity in FEF, thalamus and, via an
increase in inhibitory output from SNr, also in SC. Indeed, the
cortical microcircuit contains a variety of morphologically distinct
GABAergic neurons \citep{douglas_neuronal_2004} which could fulfil
this functionality. A similar mechanism would then be required in SC,
to reset activity generated by direct excitation via the
retinal-collicular pathway which generates express saccades. Again, SC
is a multi-layered structure, containing GABAergic interneurons
\citep{munoz_lateral_1998,meredith_intrinsic_1998,helms_organization_2004,sooksawate_organization_2011}
and there is mounting evidence that saccade dynamics are generated
within
SC \citep{kaneda_regulation_2008,goossens_optimal_2012,bayguinov_hard-wired_2015}. Thus,
a more complex treatment of the SC and FEF regions in the model may
well obviate the need for inhibitory feedback from brainstem to SC,
FEF and thalamus.}

%
% A couple of last points to round up the discussion of THIS model
%

% Reviewer 1's point 2.
\rvw{Considering whether a feedback connection, or internal,
recurrent inhibition is responsible for activity-reset in the brain
model raises a more general question about modelling the central
nervous system. We should consider whether inaccuracies within one
part of the model may propagate errors through the closed-loop system
that cannot be counteracted by another part of the simulation. There
is no way to know, from integrating sub-systems, which properties hold
true, and which are false. However, by integrating models and
examining the behaviour of the combined model, we are presented with
the right questions to ask of the model and the experimental data. In
the case of activity-reset, this is to re-assess whether there exists
inhibitory feedback from brainstem to the SC and FEF regions, and to
find out how an integrated model with self-regulatory mechanisms in SC
and FEF may perform.}

% In a model of SC, \cite{massone_neural-network_1994} suggests that
% internal mechanisms, learned via a supervised method, are sufficient
...
\rvw{We have called this closed-loop, biomimetic modelling approach \e{computational
neurobehaviour}, in which a complete, behaving model is constructed,
with attention paid to the biological accuracy of each brain and
biomechanical sub-system.
% Address final comment from Reviewer 1
\rvwf{We are by no means the first researchers to consider this
interaction between brain and biomechanics. Integrative approaches to
motor control have been referred to as \e{neuromechanics} by some
authors~\citep{nishikawa_neuromechanics:_2007}. This field appears to
have developed from detailed and low level studies of the mechanics of
muscle control systems, focussing on the neural systems `closest' to
the muscle~\citep{chiel_brain_2009}. It is evident that a recognition
of the importance of sensory input to these systems has evolved within
this field. Indeed, \cite{edwards_neuromechanical_2010} specifically
reviews closed-loop, \e{neuromechanical simulations} of behaviour in
three organisms; fly, locust and cat. The term neuromechanical
simulation \citep{pearson_assessing_2006,edwards_neuromechanical_2010}
is analogous to our computational neurobehaviour, although it does not
emphasise the important \e{behavioural} aspect of the works.}

There also exist many
closed-loop \e{robotic} systems which receive sensory input from the
world, process that input and generate behaviour by activating motor
systems
\citep{yu_development_2004,fend_artificial_2004,pearson_whiskerbot:_2007}. We
now consider whether robotic systems which model biological
components \e{in hardware} could fall within our new category.
%
Using a number of examples, we will attempt to illustrate what we mean
by computational neurobehaviour. We'll consider which examples fall
into the new category and which are covered by other fields of
robotics or computational neuroscience.}

\rvw{\cite{pearson_whiskerbot:_2007} describe a wheeled robot which has a
biomimetic whisker sensory system, along with a biomimetic neural
system imitating the operation of the rat's sensory processing and
controlling the movement of the robot. \cite{fend_artificial_2004} is
a similar, wheeled, whiskered robot, with a repertoire of three
behaviours organised in a subsumption architecture. In both robots,
actions that are selected within the brain model drive a
non-biologically accurate motor control algorithm to achieve
rotational and translational movements. Although both have sensory and
processing systems which are guided by biology, the non-biological
motor control stage prevents us from considering these as being
studies of computational neurobehaviour. Instead, we would refer to
these as \e{embodied models}, as described
in \cite{bolado-gomez_biologically_2013}, a study in which the
learning behaviour of a biomimetic `core' model is embedded within an
engineered `architecture' (a wheeled robot) which closes the
agent-environment loop.}
%
\rvw{\cite{yu_development_2004} report on a biomimetic fish robot,
whose motor system closely resembles that of the real fish. The robot
is able to operate in a closed-loop mode, where sensory input is
provided to the non-biomimetic control algorithms from overhead
cameras, but its control system is also able to operate in open-loop
mode. Neither the control system, nor the sensory system are
biologically accurate and we would not describe this study as
computational neurobehaviour. Nevertheless, the biomechanically accurate
motor system they describe has the potential to form part of a
computational neurobehavioural study of swimming behaviour in the
fish, if it were combined in a loop with suitable sensory input and
sensory processing models.}
%
\rvw{\cite{knips_neural_2017} is a report of a reach-and-grasp robot
arm controlled via a dynamic neural field brain model. The sensory
input for this system---its `eyes'---is a Microsoft Kinect sensor; it
also has somatosensory feedback from the fingers of the robot's
hand. The neural field `brain' controls the seven degrees of freedom
of the arm to carry out the reach-and-grasp action. While this robot
has closed-loop control and is clearly inspired by biology, it remains
a study of robotics and of the improvement of the control of the
robot's reach-and-grasp function, rather than a study which aims to
learn more about the biology of a primate arm. For this reason, we
would describe the study of \cite{knips_neural_2017} as an embodied
model.}

\rvw{To summarise, in most closed-loop robotic studies which
incorporate neuromimetic models, the hardware forms an `engineered
surround architecture' allowing for the examination of the behaviour
of the embodied model. However, suitably \e{biomimetic} hardware such as
the fish in \cite{yu_development_2004} would not be excluded from
computational neurobehavioural studies, especially if movement is by
biomimetic muscle actuators \citep{wilson_cerebellar-inspired_2016,ohalloran_review_2008}.}

\rvw{Modern programming platforms, often originating from the
computer game industry, make it relatively easy to model a virtual
environment. Consequently, an increasing number of studies into
robotic or neuromimetic control are carried out with virtual robots
operating within a virtual environment.}
%
\rvwf{This approach is taken in the studies described
in \cite{edwards_neuromechanical_2010}. \cite{dickson_integrative_2006}
describes an integrated model of Drosophila flight in which visual
input is processed by an algorithmic (that is, non-neural) `brain' to
control a biomechanically accurate representation of the fly. As the
virtual fly traverses its environment, its visual input updates,
closing the loop of the simulation. \cite{cope_model_2016} is a recent
study which omits the biomechanically accurate component of the model
but could provide the biologically plausible brain to make a
computational neurobehavioural fly
model. \cite{cofer_neuromechanical_2010} use an environment called
AnimatLab \citep{cofer_animatlab:_2010} to simulate the locust's
jumping mechanism in an open-loop, software-only investigation. As
this appears to be a feed-forward model without sensory feedback, we
would describe it as an input-assumption model.}
%
\rvw{\cite{nguyen_saccade_2014} and
\cite{thurat_biomimetic_2015} are two studies of the oculomotor system
which model sensory input, neural control \e{and} motor output in
software. These studies fall outside the remit of computational
neurobehaviour only because they omit to close the sensory
loop. \cite{dewolf_spiking_2016} describes a reach model comprising a
simplified virtual arm (with fewer degrees of freedom than a primate
arm), and a biologically inspired brain model. This model also omits
to close the sensory feedback loop and we consider it a computational
neuroscience study of a (virtually) embodied model.}
%

% Reviewer 1's point 4.
\rvw{There are also experimental closed-loop approaches to
understanding sensorimotor control. \cite{ejaz_closed-loop_2013}
places a fly in a fixed position, and couples it with a free-to-move
robot. The sensory input collected by the robot is projected onto the
eyes of the fly, and activity from a selected neuron in the fly's
brain is used to drive a control system for the robot's
movements. This allows the experimenters to study the behaviour of the
fly's brain operating in a closed-loop condition that is more natural
than the open-loop condition that many other experimental techniques
mandate. The results from closed-loop experiments will undoubtedly
inform future neurobehavioural models.}

%
% Now the closing section of the discussion.
%

\rvw{Thus, while there are many models that close the
agent-environment loop and display partial biological plausibility,
the biomimetic features are usually confined to a sub-system of the
entire model. This leads us to formalise a definition of computational
neurobehaviour as: \e{The study of biological sensory-motor system
behaviour using biologically accurate models of sensory input, brain
and motor sub-systems operating in a closed-loop}.}
%
% Another computational neurobehavioural study?
\rvw{We believe our oculomotor model is one of the first such models
using this approach and shares many features with that
of \cite{arena_motor-skill_2017} which describes a robotic insect
system based on the fly species \e{Drosophila Melanogaster}. It has
biomimetic insect legs implemented in a virtual robot and a
neuromimetic brain. Closing the loop is a visual sensory input system
which is able to determine the distance to an
on-coming obstacle and the obstacle's height. The authors demonstrate that the
virtual robot is able to learn to climb in a realistic
manner and suggest it may be compared with experimental
data from future Drosophila experiments addressing obstacle climbing
and learning. The visual system is not described in detail, but if it
is modelled in a biologically plausible manner, then this work may
reasonably be described as a computational neurobehaviour study,
contemporaneous with our own.}
...
%% biological system and closes the loop in this way, enabling the
