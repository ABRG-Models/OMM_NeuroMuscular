...
\subsubsection{Co-ordinates in the world} \label{sec:methods:coords}
...
into a `sustained retinal' signal. The justification here is that the
model reacts to simple luminant targets and does not need to carry out
the feature extraction performed by these visual areas. The
supplementary eye fields are involved in the programming of saccade
sequences \citep{tehovnik_eye_2000} and memory guided
...
from the model.}
%
% Figure here is the ``macroscopic architecture'' one.
%

% Now describe each bit a little more, with help of the figure.
...
iterdigitated poulations of projection neurons expressing primarily D1
...
is reduced. The selection mechanism of the GPR model is the
...
channels that have strong saliencel (input) have weak output at the
...
from STN, which effectively supplies a sum of the STN inputs to each
...
$c$ (see Eq.~\ref{eq:lintransfunc}). In striatum, Str\_D1 and Str\_D2
have positive offset $c$, mimicking the so-called `down-state' of
medium spiny neurons which have a resting potential far below spiking
threshold and require co-ordinated input to generate action
potentials \citep{wilson_origins_1996}.
% Note, in Cope's paper, there is an error with the offsets for
% D1,D2. These are set to -0.05 instead of 0.05. A quick trial with
% the model suggests this doesn't make a very large difference to the
% operation, and I have the sign correct in any case.
...
activation as described below in Eqs~\ref{eq:a_in_d1} \& \ref{eq:a_in_d2}.}


% Change this - we already talked about channels?
\rvwt{The \ccg~model adopts a `channel based' model from previous
work~\citep{redgrave_basal_1999,gurney_computational_2001}.
Within the context of the basal ganglia model, a `channel' is a
sub-set of neurons which are associated with an individual
action. Each channel in BG loops back via relays in the thalamus
through cortex. Input into this thalamo-cortical loop is termed
`salience'. Because it is a loop, input salience to the channel may
originate cortically or sub-cortically. The higher the salience of the
channel, the greater is the behavioural relevance of the associated
action.}

\rvwt{In the oculomotor model, an action
corresponds to a distinct saccade end point. It can be seen that the
action space for reflexive saccade end points is therefore
continuous rather than discrete. MORE HERE}

% FEF stuff taken from Alex's pre-pub
...
between them. Each population of 2500 elements is arranged as a 50 by
50 grid which forms a map in a retinotopic co-ordinate system, roughly
matching the known layout of the superior
colliculus \citep{robinson_eye_1972}.
...
{ The macroscopic architecture of the \ccg~model. The main nuclei
modelled as brain systems are: basal ganglia (BG), frontal eye fields
(FEF), thalamus (THAL) and superior colliculus (SC). The retinal input
is presented via non-biologically plausible retinal populations. The
loops through basal ganglia, which define the architecture, are shown
with coloured lines: the cortical loop (through FEF and THAL) in green
and the sub-cortical loop (through SC and THAL) in blue. Connections
with arrowheads indicate excitatory connections, those with circles
are inhibitory. A red border indicates that the box represents a
sub-system of two or more populations; a black box indicates,
within the context of the model, a single neural population.}
\end{figure}
