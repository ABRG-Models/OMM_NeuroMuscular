...
intensity)} on a topographic map. \rvwt{Whilst certain assumptions
...
colliculus. Excitatory connections are indicated with arrow heads;
inhibitory connections with circles in place of the arrow heads. The
...
work \citep{gurney_computational_2001,gurney_computational_2001-1} and is referred to as the GPR
model. The GPR model incorporates the following main components of the
primate BG \citep{mink_basal_1996,wickens_basal_1997}: (i) The
striatum (the main input station to the BG) which is divided into two
iterdigitated populations of projection neurons expressing primarily D1
or D2-type dopaminergic receptors (named Str\_D1 and Str\_D2); (ii)
The subthalamic nucleus (STN); (iii) the external segment of the
globus pallidus (GPe); (iv) the output nucleus relevant for saccadic
...
from STN, which effectively supplies a sum of \e{all} of the STN inputs to each
channel in SNr, will provide so much excitation that Str\_D1 may
become unable to inhibit one of the channels in SNr and selection may
become
...
levels \citep{chevalier_disinhibition_1990,delong_primate_1985,kita_intracellular_1991}. This
...
clearly describe the dynamic effect of the dopamine parameter as being
due to any single population, because the activity is recurrently
...
each unit in Str\_D1 projected to a counterpart SNr$_j$ in SNr with
some weight $w_{max}$, but also connected to neighbouring nodes in SNr
with a weight given by $w_{max}.G(d)$, where $G(d)$ is a circularly
symmetric, 2D-Gaussian which is a function of distance $d$ from
SNr$_j$ [Fig.~\ref{fig:ccg_model}(c)]. A similar scheme applied for
...
of the model, based on a diagram output directly from
SpineCreator. Populations for Str\_D1, Str\_D2, STN, SNr and GPe are
...
Saccadic targets are mapped retinotopically over its
surface \citep{robinson_eye_1969,bruce_primate_1985,sabes_parietal_2002},
and increased neural activity at a location in the map precedes a
...
\rvwt{FEF neurons can be divided in to three functional groups,
...
model to a direct connection with delay.}
...
adjustement \citep{latto_effects_1977}, albeit with some persistent
...
divided into the `superficial' and `deep' layers, which have
...
visual input via cortical (FEF) and sub-cortical (thalamus)
...
from retinotopic co-ordinates into the Cartesian co-ordinates. The
...
a prosaccade task in which a fixed luminance point is fixated by teh
...
as possible the weights used in the original paper. Further details on
the parameterisation of the \ccg~model are in \cite{cope_basal_2017}.}
...
The \e{LINlinear} component governs the activation $a$ with a first order
...
\rvwt{based on the D1MSN or D2MSN component}. The value of the activation
...
Figure \ref{fig:bioeye}, while Figure \ref{fig:bioeye} depicts
...
`World' population (named to mean the `world as the eye sees it',
...
in the deep layer of superior colliculus. This activity excites
activity in the correct proportions within the 6 channels of the
saccadic burst generator whose output state, the motoneurons send a
rate-code signal (normalised between 0 and 1) into the biomechanical
eye model. The rotational state of the eye model is fed back to
participate in the computation of the retinotopic luminance activity
in `World', completing the loop.}
...
hill moves to the $r=$20\dg~location on the colliculus, \e{and also
...
coordinates, and not in degrees or radians; 1$\leq\phi\leq$50
corresponds to a range of 0\dg~to 360\dg; $\phi=$38.5 corresponds to
movements left. The connections strength drops away sinusoidally as
$\phi$ moves away from the centre line at $\phi=$38.5. In regions of
...
along $\phi=$13.5. The map is a mirror of
Fig.~\ref{fig:weightmaps}(a), reflected about the line $\phi$=26. The
...
$\phi=$1; for `down' $\phi=$26. Based on the training described in
...
Fig.~\ref{fig:doublesteps}, where our small eccentricity luminance was at
8\dg~and our more distal luminance was at 12\dg~(both to the right of
centre). \rvwt{The stimuli could not be presented at 15\dg and 30\dg
to match the experiment, because 30\dg saccades were outside the range
of the model.}
...
biophysically accurate and we would not describe this study as
computational neurobehaviour. Nevertheless, the biophysically accurate
...
incorporate neuro-mimetic models, the hardware forms an `engineered
...
robotic or neuro-mimetic control are carried out with virtual robots
...
neuro-mimetic brain. Closing the loop is a visual sensory input system
