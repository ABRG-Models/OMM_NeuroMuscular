...
SNr$_j$ [Fig.~\ref{fig:ccg_model}(c)]. A similar scheme applies for
...
the head model used in the proposed framework.}
...
successful horizontal saccade of 10\dg. Activity from the 2D Gaussian,
passing through the weight maps will excite the superior and inferior
rectus channels by an equal, balanced amount, so these cancel out,
...
becomes smaller}, we can still argue that the vertical component
...
the smaller hill. However. Now consider a 2-D Gaussian hill which
codes for a 10\dg~saccade which is `up, and to the right' in equal
proportions. That means that the hill will sit on the boundary between
the weight maps for the `up', and the `right' muscles. Now, if the
hill moves to the $r=\mathrm{20}$\dg~location on the colliculus, \e{and also
...
and the sinusoidal component of the weight map.}
% Add figure for this.
