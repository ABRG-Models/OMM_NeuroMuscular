...
%
\rvwt{Reflexive saccades are elicited by peripheral changes in the visual
scene and are distinct from voluntary saccades. Reflexive saccades can
occur also as a result of auditory and somatosensory stimuli, but
these modalities are ignored in this work and
in \cite{cope_basal_2017}. The functionality reproduced by
the \ccg~model is `the selection of the best target end-point for a
reflexive saccade'. A competition such as this between competing,
incompatible movements is often referred to as an \e{action selection}
problem. The brain must choose one location to foveate, because the
eyes of most animals can only look in one direction at a time.} % Maybe cite norman_attention_1986, maes_dynamics_1989 on action selection.

\rvwt{It is thought that in vertebrates, the basal ganglia (BG) play an important
r\^ole in action
selection~\citep{redgrave,graybiel,grillner,hikosaka after the
evolutionary paper}. The \ccg~model
places the BG at the centre of the oculomotor system, adopting a
`channel based' model from previous
work~\citep{gurney_basal_2004,etc}. Within the context of the basal
ganglia model, a `channel' is a sub-set of neurons which are
associated with an action. In the oculomotor model, an action
corresponds to a distinct saccade end point. It can be seen that the
action space for reflexive saccade end points is therefore a
continuous space, rather than being selection between distinct
acts. Etc etc. }
