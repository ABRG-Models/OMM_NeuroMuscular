...
% on to Fig 3.
\rvwt{The GPR model in Fig.~\ref{ccg_model}(b) has only three
...
this diagram with Figs.~\ref{ccg_model}(a) \&
(b). Fig.~\ref{brain_model} expands the `SC', `BG' and `slow retinal'
boxes from Fig.~\ref{ccg_model}(a).}
...
(Fig.~\ref{mapping}, red cross). This makes it virtually impossible
for a peripheral target (Fig.~\ref{mapping}, yellow cross) to win
...
Fig.~\ref{sbg}. \rvw{In the brainstem model, we use the word `channel'
