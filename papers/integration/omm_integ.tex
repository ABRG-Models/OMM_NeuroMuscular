...
% Describe what is modelled first
...
\rvwt{One hypothesis for the r\^ole played by the basal ganglia (BG)
is that the system performs \e{action selection}
...
following the known anatomy of the
region \citep{hikosaka_role_2000}. The BG receives input indirectly
from the superior colliculus, which has a retinotopic
arrangement \citep{ottes_visuomotor_1986}.
%and a signal which is associated with saccadic eye
%movements \citep{wurtz_superior_1971}, specifying the saccade
%end-point.
The BG receives excitatory
inputs directly from retinotopic regions of the cortex including the
frontal eye fields (FEF), supplementary eye fields (SEF), lateral
intraparietal cortex (LIP) and dorsolateral prefrontal cortex. The
dorsolateral prefrontal cortex, which participates in voluntary
saccades
...
at medium spiny neurons with D1 receptors \citep{hernandez-lopez_D1_1997,gonon_prolonged_1997} and reducing
transmission at those with D2 receptors \citep{delgado_inhibitory_1999}. These effects are
modelled using a dopamine parameter which modulates the input
activation as described below in
Eqs~\ref{eq:a_in_d1} \& \ref{eq:a_in_d2}; Str\_D1 activation is enhanced;
Str\_D2 activation is suppressed.}

% Change this - we already talked about channels in the context of the
% three channel model show. Perhaps modify to talk about how we have a
% map of channels - almost a continuous space in the \ccg model. This
% can then lead on to Fig 3.
\rvwt{The GPR model in Fig.~\ref{ccg_model}(b) has only three
channels with focussed inhibition from striatum to SNr and GPe defined
by a simple one-to-one scheme. The action channels represent discrete,
incompatible motor action choices. In the oculomotor model, an action
channel might represent the end-point of saccade, and the competition
carried out in the basal ganglia is between potential saccade
end-points. However, eye movements have a \e{continuous} end-point
space; the eye can rotate to any orientation within its
biomechanically permissible range. Some end-points within this range
are certainly mutually exclusive---it's not possible to look to the
left and to the right simultaneously---but \e{nearby} end-points are
not necessarily incompatible. A small enough error in the end-point of
a saccade will not prevent the eye from foveating on a target as the
fovea is not infinitesimally small. To cope with this requirement,
the populations within the oculomotor basal ganglia are conceived of
as two-dimensional topographic grids of neural elements. Activity in
each element corresponds to a spatial location in the visual
field. \cite{someref} here.}

% Now some FEF stuff taken from Alex's pre-pub
\rvwt{Add description of FEF. (FIXME).}
...
between them. \rvwt{Describe this figure in more detail. (FIXME).}
