...
% Existing models don't control movement unless they control either a mechanical
% or virtual body.
% Can't observe how learning in the model may effect it's evolution in time
% We may miss features about the system
% Brains exist to move muscles

% Solution, build an integrated model. Also give background on OMM system here.
...
\citep{seth_opensim:_2011,seth_opensim_2015}, is anatomically represented by a sphere of
...
%
% This doesn't account for *all* cases where you'd want to remove the TN
% activity. Imagine a saccade purely UP then a saccade to a location
% purely RIGHT. You'd need to remove the TN activity on the UP channel,
% but you'd not be inducing any DOWN with the new target.
...
%\subsection{Saccade with distractor} enough!

%Our integrated model allows us to examine the effect of distractor
%luminances on saccade dynamics. Fig.~\ref{targ_w_distr}(a) shows
%three horizontal saccades to a
%target at $\theta_{y}=$-10\dg. The first saccade (blue
...
%located at $\theta_{x}=\,$-7\dg, $\theta_{y}=\,$-7\dg.
%The distractor appeared 0.1~s after the target and
%persisted for 0.1~s [Fig.~\ref{targ_w_distr}(b)]. The third
%saccade (green) had conditions identical to the red saccade, but
%with a brighter distractor luminance, whose magnitude was 2.

%\cmnt{Ok, now saccades can deviate TOWARDS a distractor - particularly
%if the distractor is closer to the trajectory - see for example
%~\cite{wang_distractor_2014}. This model doesn't seem to have any
%mechanism to induce a deviation of trajectory away from the distractor. }

%\subsection{Saccade to twin targets} enough!
...
% This is probably recapitulation of intro material.
...
%\end{center}
% \textbf{\refstepcounter{figure}\label{01} Figure \arabic{figure}.}{ Enter the caption for your figure here.  Repeat as  necessary for each of your figures }
%\end{figure}
...
%\end{center}
%\textbf{\refstepcounter{figure}\label{02} Figure \arabic{figure}.}{ Enter the caption for your figure here.  Repeat as  necessary for each of your figures }
%\end{figure}
...
%{ \textbf{intro\_graphic} An isolated brain model. Crafted inputs
...
which make up \stob, but the hand-written components must be
...
{ Representative mapping from eye's frame of
...
{ Accuracy at different target eccentricities for fixation luminance 0.2 and target luminance 0.3.}
\end{figure}
...
{ Weight maps for the connections between the
...
{ The end-point error surface for the model
...
{ Representative single saccades. a) Trajectories
...
{ Double steps. The effect of illuminating a first target
...
%\end{center}
%\textbf{\refstepcounter{figure}\label{targ_w_distr} Figure \arabic{figure}.}
%{ \textbf{targ\_w\_distr} Effect of a distractor on single saccade.}
%\end{figure}
