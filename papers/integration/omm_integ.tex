...
\cmnt{Background, gap, solution (cheers Sean!)}
...
\cmnt{Note that inhibition from IBN populations is fed back - or is this
...
% Patras - what about a figure here? --- WILL ADD TOMORROW TUESDAY
...
often executed \e{more quickly} than if it were programmed on its own.

%\subsection{Saccade with distractor} enough!

%Our integrated model allows us to examine the effect of distractor
%luminances on saccade dynamics. Fig.~\ref{fig:targ_w_distr}(a) shows
...
axes (Fig.~\ref{fig:errorsurfaceTM3}). Thus, \e{the integration of
...
\cmnt{Need to remark that component stretching has not been addressed.}



\cmnt{Omitted main sequence?}

\cmnt{Explain significance of this work: We have integrated several models }

\cmnt{Conclude with a statement something like `the Oculomotor system is well studied, with much being known about its operation, but it remains that a full understanding of this brain and biophysical system is far from complete.'}

% Be sure to say that ``in future work, it will be possible to
% investigate saccade trajectory modifications in Parkinson's'' - Jen's model.
...
\cmnt{Get RRID for various software tools; SpineCreator Resource
ID: SCR\_015637 SpineML\_2\_BRAHMS (SCR\_015640) SpineML (SCR\_015641) BRAHMS (SCR\_015642)}

\cmnt{Could make movies as supp. data, too.}
