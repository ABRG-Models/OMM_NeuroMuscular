...
#
# For Model1 to show effect of the changing hill of activity size.
#
# _Horizontal_ movements (Rotn about Y)
#
...
EOF
    ...
    cat >> script${targyval}.sh <<EOF
env
pushd ${HOME}/OMM_NeuroMuscular/batch_scripts/expt_sacc_vs_targetpos_M1_horz
...
EOF
    ...
    qsub ${PROJECT_TAG} -v OMMODEL=${OMMODEL} -N SVTP${targyval} -wd ${HOME}/OMM_NeuroMuscular/batch_scripts/expt_sacc_vs_targetpos_M1_horz -o results/SVTP${targyval}.out -j y ./script${targyval}.sh
