...
function model_data = load_ocm (output_base_path)
    ...
    [fef_add_noise, count] = load_sc_data ([model_log_path 'FEF_add_noise_out_log.bin'], 2500);
    [fef, count] = load_sc_data ([model_log_path 'FEF_out_log.bin'], 2500);
    [strd1, count] = load_sc_data ([model_log_path 'Str_D1_out_log.bin'], 2500);
    [strd2, count] = load_sc_data ([model_log_path 'Str_D2_out_log.bin'], 2500);
    [stn, count] = load_sc_data ([model_log_path 'STN_out_log.bin'], 2500);
    [snr, count] = load_sc_data ([model_log_path 'SNr_out_log.bin'], 2500);
    [gpe, count] = load_sc_data ([model_log_path 'GPe_out_log.bin'], 2500);

    [world, count] = load_sc_data ([model_log_path 'World_out_log.bin'], 2500);

    [ret1, count] = load_sc_data ([model_log_path 'Retina_1_out_log.bin'], 2500);
    [ret2, count] = load_sc_data ([model_log_path 'Retina_2_out_log.bin'], 2500);

    [scd, count] = load_sc_data ([model_log_path 'SC_deep_out_log.bin'], 2500);

    % If second SCD layer exists:
    scd2path = [model_log_path 'SC_deep2_out_log.bin'];
    havescd2 = 0;
    if (exist(scd2path, 'file') == 2)
        display(['Have SCDeep2 log file at ' scd2path]);
        havescd2 = 1;
        [scd2, count] = load_sc_data (scd2path, 2500);
    else
        scd2 = [];
    end

    scd3path = [model_log_path 'SC_deep3_out_log.bin'];
    havescd3 = 0;
    if (exist(scd3path, 'file') == 2)
        display(['Have SCDeep3 log file at ' scd3path]);
        havescd3 = 1;
        [scd3, count] = load_sc_data (scd3path, 2500);
    else
        scd3 = [];
    end

    [scd_tt, count] = load_sc_data ([model_log_path 'SC_deep_to_SC_deep_Synapse_1_postsynapse_out_log.bin'], 2500);

    [scs, count] = load_sc_data ([model_log_path 'SC_sup_out_log.bin'], 2500);
    %    [scsm, count] = load_sc_data ([model_log_path ...
    %                        'SC_smoothing_a_log.bin'], 2500);
    [sca_in, count] = load_sc_data ([model_log_path 'SC_avg_in_copy_log.bin'], 2500);
    [sca, count] = load_sc_data ([model_log_path 'SC_avg_out_log.bin'], 2500);

    [thal, count] = load_sc_data ([model_log_path 'Thalamus_out_log.bin'], 2500);

    [r2_to_r1, count] = load_sc_data ([model_log_path 'Retina_2_to_Retina_1_Synapse_0_postsynapse_out_log.bin'], 2500);

    [w_to_r1, count] = load_sc_data ([model_log_path 'World_to_Retina_1_Synapse_0_postsynapse_out_log.bin'], 2500);
    ...
    keySet = {'r2_to_r1', 'w_to_r1', 'fef_add_noise', 'fef', 'strd1', ...
              ...
              'eyeTime', 'opn', 'scsum', 'llbn_l', 'llbn_in', 'ebn', 'tn', 'mn', 'ibn', ...
              'llbn_r', 'llbn_in_r', 'llbn_u', 'llbn_d', 'llbn_zp', ...
              'llbn_zm', 'ebn_r', 'tn_r', 'mn_r', 'ibn_r' ...
              'wm_l', 'wm_r', 'wm_u', 'wm_d', 'wm_zp', 'wm_zm', 'scd2', 'scd3'};
