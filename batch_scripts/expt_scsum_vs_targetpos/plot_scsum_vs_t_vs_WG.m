...
colours = {'r','b','g','k','c','m',[0.6 0.4 0.8],[0.0 0.8 0.5]};
colcount = 1;

fixed_legend = ['All 5 WG';'minus FEF->SC_d';'minus recurrent SC_d';'minus SC_s->SC_d';'minus FEFaddnoise->FEF';'minus SC_s->Thal';'re-add FEFaddnoise->FEF';'Re-add SC_s->Thal'];

for j = [4 5 6 7 8 9 10 11]

     globstr = sprintf('results/WG%.0f/r*.dat', j);
     flist = glob(globstr);
     llen = size(flist)(1);
     for i = [1 : llen]
...
for w = wfs
    ...
    legend_str = [legend_str;  'WG' num2str(w)];
end
resizegraph (62, 1024, 1024);
plot ([-15,-8],[-15,-8], 'g--');
xlabel('Target y');
ylabel('eyeRy');
legend(fixed_legend,'location','southeast');
...
for w = wfs
    ...
    set(pl,'color',colours{colcount});
    legend_str = ['WG' num2str(w)];
end
resizegraph (65, 1024, 1024);
xlabel('Target y');
ylabel('Latency (ms)');
legend(fixed_legend,'location','northwest');
...
%resizegraph (66, 1024, 1024);
title ('Mean SCSUM activity')
xlabel ('TargY');
ylabel ('time (ms)');
zlabel ('SCSUM activity');
legend ('colours as in other graphs');
