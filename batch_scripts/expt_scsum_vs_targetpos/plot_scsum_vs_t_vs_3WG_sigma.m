...
for j = [15 20 25 35] % 35 is 25 but with normfactor 1.4!!! FIXME
                      % Just a quick look at dat aon Thurs afternoon

     globstr = sprintf('results/WG_three/sigma%.0f/r*.dat', j)
     flist = glob(globstr);
     llen = size(flist)(1);
     for i = [1 : llen]
         ...
         rnm = flist{i};
         if (j<10)
             resdatname = substr(rnm, 25);
         else
             resdatname = substr(rnm, 26);
         end
