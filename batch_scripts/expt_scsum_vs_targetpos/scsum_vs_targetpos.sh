...
# NUM_RUNS of the model for each target. Results are the SC_sum activity
...
EOF
    ...
    cat >> script${targyval}.sh <<EOF
env
pushd ${HOME}/OMM_NeuroMuscular/batch_scripts/expt_scsum_vs_targetpos
...
octave -q --eval "perform_sacc_scsum('./results',${THETAX},${targyval},${NUM_RUNS},${GAP_MS},${LUMVAL},${DOPAMINE})"
exit 0
EOF
    ...
    qsub ${PROJECT_TAG} -v OMMODEL=${OMMODEL} -N SVTPY${targyval} -wd ${HOME}/OMM_NeuroMuscular/batch_scripts/expt_scsum_vs_targetpos -o results/SVTP${targyval}.out -j y ./script${targyval}.sh
