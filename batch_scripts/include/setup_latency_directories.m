function directories = setup_latency_directories (params, counter)

    output_dir = ['/fastdata/' getenv('USER') '/ocmRX' ...
                  num2str(params.targetThetaX) 'RY' num2str(params.targetThetaY) ...
                  '_FOFF' num2str(params.fixOff) '_TON' num2str(params.targetOn) ...
                  '_FL' num2str(params.fixLuminance) ...
                  '_TL' num2str(params.targetLuminance) ...
                  '_D' num2str(params.dopamine) '_' num2str(counter)];
    qlog_dir = output_dir;

    output_model_dir = [output_dir '/model'];
    output_run_dir = [output_dir '/run/'];
    output_log_dir = [output_dir '/log/'];

    directories = struct ('root', output_dir, ...
                          'qlog', qlog_dir, ...
                          'model', output_model_dir, ...
                          'run', output_run_dir, ...
                          'log', output_log_dir ...
                          );
end
