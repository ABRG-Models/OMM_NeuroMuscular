...
EOF
    ...
    cat >> script${targxval}.sh <<EOF
env
pushd ${HOME}/OMM_NeuroMuscular/batch_scripts/expt_sacc_vs_targetpos_M1
...
EOF
    ...
    qsub ${PROJECT_TAG} -v OMMODEL=${OMMODEL} -N SVTP${targxval} -wd ${HOME}/OMM_NeuroMuscular/batch_scripts/expt_sacc_vs_targetpos_M1 -o results/SVTP${targxval}.out -j y ./script${targxval}.sh
