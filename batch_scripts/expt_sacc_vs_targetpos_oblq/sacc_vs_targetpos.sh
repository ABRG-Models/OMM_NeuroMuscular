...
#
# Oblique version - for down/right saccades.
#
...
octave -q --eval "perform_saccade('./results/${OMMODEL}',${targyval},${targyval},${NUM_RUNS},${GAP_MS},${LUMVAL},${DOPAMINE})"
exit 0
EOF
    ...
    qsub ${PROJECT_TAG} -v OMMODEL=${OMMODEL} -N SVTPY${targyval} -wd ${HOME}/OMM_NeuroMuscular/batch_scripts/expt_sacc_vs_targetpos_oblq -o results/SVTP${targyval}.out -j y ./script${targyval}.sh
